...
function createAdminEngine(env) {
  ...
  const toolExecutor = new ToolExecutor({ toolRegistry: registry, metricsCollector });
...
export async function handleAdminAiAction(request, env) {
  try {
    ...
    const changes = (result.results || []).filter(r => r.success).map(r => ({
      toolName: r.toolName,
      data: r.data,
      success: true,
    }));
    const errors = (result.results || []).filter(r => !r.success).map(r => ({
      toolName: r.toolName,
      error: r.error,
      success: false,
    }));
...
export async function handleAdminConversations(request, env) {
  try {
    ...
    if (url.pathname === '/api/admin/conversations' && request.method === 'POST') {
      const body = await request.json();
  ...
  try {
    ...
    if (history && Array.isArray(history)) {
      for (const msg of history) {
        if (!conv.history.find(h => h.content === msg.content && h.role === msg.role)) {
...
Historial:
${conv.history.slice(-10).map(m => `[${m.role}]: ${m.content}`).join('\n')}

Responde SOLO con JSON:
{"suggestedReply": "texto", "suggestedAction": "texto", "suggestedTools": ["tool1", "tool2"]}`;

    const raw = await chat(env, [
      { role: 'system', content: 'Eres un asistente que sugiere respuestas para conversaciones de atención al cliente.' },
      { role: 'user', content: prompt },
    ]);
    ...
    try {
      ...
      suggestions = match ? JSON.parse(match[0]) : { suggestedAction: 'Revisar conversación manualmente' };
    }
