...
describe('ToolExecutor Integration', () => {
  it('executes tool with metrics collector', async () => {
    ...
    const executor = new ToolExecutor({ toolRegistry: registry, metricsCollector: mc });
  ...
  it('tracks duration', async () => {
    ...
    registry.register({
      name: 'slow',
      execute: async () => { await new Promise(r => setTimeout(r, 5)); return 'done'; },
    });
    await executor.execute('slow');

    const toolMetrics = mc.snapshot().byTool.slow;
    expect(toolMetrics.totalDuration).toBeGreaterThanOrEqual(5);
  });
  ...
  it('validates required params through executor', async () => {
    ...
    registry.register({
      name: 'strict',
      inputSchema: { name: { type: 'string', required: true }, age: { type: 'number', required: true } },
  ...
  it('passes context to tool execution', async () => {
    ...
    await executor.execute('ctxTool', { x: 1 }, { sessionId: 's1', clientId: 'c1' });
    expect(executeFn).toHaveBeenCalledWith({ x: 1 }, { sessionId: 's1', clientId: 'c1' });
  });
