...
describe('NexusAIEngine', () => {
  ...
  it('rejects tools not in profile', async () => {
    ...
    const engine = new NexusAIEngine({ chatFn });
    engine.toolRegistry.register({ name: 'restrictedTool', execute: async () => 'ok' });
  ...
  it('adds assistant message to history', async () => {
    const chatFn = vi.fn().mockResolvedValue(JSON.stringify({ plan: [], explanation: 'Bot reply' }));
    const engine = new NexusAIEngine({ chatFn });
  ...
  it('accepts custom initial session data', async () => {
    const chatFn = vi.fn().mockResolvedValue(JSON.stringify({ plan: [], explanation: 'ok' }));
    const engine = new NexusAIEngine({ chatFn });
    await engine.process('test', { sessionId: 'init-session', clientId: 'c-init', conversationId: 'conv-init' });
