...
export async function fetchAllPublicData() {
  const endpoints = [
    'business-info', 'categories', 'services', 'prices', 'promotions',
    'warranties', 'print3d', 'faqs', 'social-media', 'phones',
    'address', 'featured-messages', 'hours',
  ];
  const results = await Promise.allSettled(
    endpoints.map(async name => ({ name, data: await fetchPublic(name) }))
  );
  const data = {};
