...
| Interview | Recolecta datos estructurados para reparación: nombre, teléfono, dispositivo, problema | ✅ Implementado | `services/interview/v2/` |
...
| `interview-router.js` | Puente entre Nexus e Interview. Detecta si hay entrevista activa. |
...
| Cómo se interpreta, valida y avanza una respuesta de entrevista | `services/interview/v2/interview-controller.js` | `answerMessage()` y el resultado con `question`, `interviewComplete` y `summary` | `interpreter.js` → `resolver.js` → `question-generator.js` |
...
particular, las reglas de Interview, tools, acceso a datos y WhatsApp.
...
- `interview_sessions` — sesiones activas de entrevista
